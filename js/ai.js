...
window.AI = (function () {
  ...
  /* 提示词：融合了两个教育 skill 的方法论
   *   1) photo-question-tutor（拍照答疑）：识题 → 条件·目标·方法·步骤讲解 → 易错提醒 → 同类变式题，
   *      硬性要求「不得只输出答案」「识别不确定必须标注」「讲解匹配年级」；
   *   2) agent-mistake-review（错题复盘）：判错因 → 一句话订正 → 同类题 → 间隔复习安排，
  ...
  function buildPrompt(subject, grade, note) {
    ...
    return '你是初中' + subj + '老师，学生拍了这张' + gradeName + subj + '错题照片。请直接下结论并讲解。\n' +
      ...
      'C. 严禁出现「让我再看」「等等」「不对」「重新看」「假设」「尝试」这类自我怀疑、反复推翻的字句。\n' +
      'D. steps 是写给学生看的最终解法：3 步，每步不超过 45 字，只写结论性推导。\n' +
      'E. 全文不超过 600 字。写完 JSON 立即停止。\n\n' +
      '【讲解要求】\n' +
      ...
      '【输出模板 · 字段顺序照抄】\n' +
      '{"question":"","studentAnswer":"","answer":"","knowledge":"","causeType":"","causeDetail":"",' +
  ...
  function salvageJSON(text) {
    ...
    ['question', 'studentAnswer', 'answer', 'knowledge', 'causeType',
   ...
   * 宁可多滤：留下 2 条干净步骤，也好过 6 条「再次复核图…不可能…」的推敲残渣。 */
  var META_RE = /(让我|我们(来|再)|等等|不对|重新|再次|复核|破局|假设|尝试|修正|我(算|看|想)错|是不是|不可能|如果是|如果这样|意味着|这说明|隔断了|[?？])/;
  ...
  function sanitizeSteps(steps) {
    ...
    steps.forEach(function (s) {
      ...
      t = t.replace(/^[：:、,，.。]\s*/, '');        // 剥掉残留的「：」
      if (!t) return;
      if (META_RE.test(t)) return;                 // 自我怀疑 / 自问自答的句子直接丢掉
   ...
   * 这里把输出压到 5 个字段（实测几十 token 就能答完），模型没有空间跑飞，因此几乎不会失败；
   * 步骤仍然用第一次抢救回来的内容，答案/题目/知识点用这次的结果覆盖，取长补短。 */
  function buildRetryPrompt(subject, grade, note) {
    ...
    return '你是初中' + subj + '老师。看这张' + Store.gradeName(grade) + subj + '错题照片，' +
      ...
      (n ? '学生备注：「' + n + '」，只处理这道题。\n' : '') +
      '不要写「让我」「等等」「不对」「假设」这类句子。若看不清或不确定，把 uncertain 设为 true，answer 给出最可能的答案。\n' +
      '{"question":"题目(含已知与所求)","studentAnswer":"学生原答案(看不出填空串)","answer":"正确答案","knowledge":"考查知识点","uncertain":false}';
  }
  ...
  function runOnce(messages, opts) {
    return chat(messages, opts).then(function (j) {
      ...
      var result = {
        schema: 'v2',                 // v2 = 含错因/同类题/复习建议（v1 为旧版，前端兼容两者）
        ...
        .then(function (r) {
          ...
          return runOnce(msgs(buildRetryPrompt(subject, grade, note)), { max_tokens: 800, note: note })
            .then(function (r2) {
              ...
              // 取长补短：讲解内容用第一次的，题目/答案/知识点用第二次（更可靠）的
              var m = {};
