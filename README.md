...
| 讲题 | 复盘 |
|---|---|
| 题目、学生原答案、正确答案、**为什么这样做**、**分步讲解**（每步带依据）、知识点、易错点 | 错因分类 + 依据、一句话订正、同类变式题、复习安排（几天后重做） |

提示词融合了三个教育 skill 的方法论：`agent-question-explanation`（AI 讲题）的
"为什么这样做 + 分步讲解"（v3 把这两块升级为一等字段）、`photo-question-tutor`（拍照答疑）的四步讲解与
...
1. **提示词铁律**（`js/ai.js buildPrompt`）：`最多复核一次`；自算结果与图中标注答案不一致时
   直接标 `uncertain: true` 而不是反复重算；禁止「让我/等等/不对」这类字句；`steps` 限 3-5 步 60 字（每步要写清做什么和依据）。
...
```json
{
  ...
  "answer": "最终答案",
  "why": "为什么这样做：方法选择的依据、怎么想到这个方法",
  "steps": ["第一步（做什么 + 依据）", "第二步..."],
  "knowledge": "考查知识点",
...
> `schema: "v3"` 是把 steps 升级为「分步讲解、每步含依据」并新增 `why`（为什么这样做）的版本；
> `why` 是教学价值最高的短字段，所以排在 JSON 里 `answer` 紧后面、`steps` 仍然在最末尾——
> 输出被截断时先丢步骤，不会丢答案也不会丢 why。`quality` / `warnings` 是质量自检：
> 模型跑飞（输出被截断 / 步骤里全是自我推敲 / 解不出答案）时不假装成功，把问题交给界面提示。
> 老记录（v1/v2）没有 `why` 字段，详情页条件渲染，不会出现 `undefined`。
...
| 脚本 | 覆盖 | 跑法 |
|---|---|---|
| `detail_test.js` | 详情页渲染（v3 分析字段、质量提示条、他人解答区块、XSS 转义）+ 提示词内容 | `node tests/detail_test.js` |
